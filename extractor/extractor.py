...
@click.option("-e", "--ext", "exts", multiple=True, help="源代码后缀，可以指定多个，默认为Python源代码")
@click.option(
    "-c", "--comment-char", "comment_chars", multiple=True, help="注释字符串，可以指定多个，默认为#、//"
)
@click.option("--font-name", default="宋体", help="字体，默认为宋体")
@click.option(
    "--font-size", default=10.5, type=click.FloatRange(min=1.0), help="字号，默认为五号，即10.5号"
)
@click.option(
    "--space-before", default=0.0, type=click.FloatRange(min=0.0), help="段前间距，默认为0"
)
@click.option(
    "--space-after", default=2.3, type=click.FloatRange(min=0.0), help="段后间距，默认为2.3"
)
@click.option(
    "--line-spacing", default=10.5, type=click.FloatRange(min=0.0), help="行距，默认为固定值10.5"
)
@click.option(
    "--exclude",
    "excludes",
...
def main(
    ...
    excludes = del_slash([abspath(exclude) for exclude in excludes] if excludes else [])

    # 第一步，查找代码文件
    finder = CodeFinder(exts)
    files = [file for indir in indirs for file in finder.find(indir, excludes=excludes)]
